          ...
          (progn
            ...
            (cl-letf (((symbol-function 'call-process)
              ...
              (should (equal (car calls)
                             (list (list edger-executable nil nil nil "cross" "right" "split")
...
(ert-deftest edger-confirm-only-when-axis-is-split ()
  (save-window-excursion
    (delete-other-windows)
          ...
          (progn
            ...
            (cl-letf (((symbol-function 'call-process)
              ...
              (let ((top (selected-window)) (bottom (split-window-below)))
                ...
                (edger-down)
                (should (equal (last (car calls) 3) '("cross" "down" "split")))
                (edger-right)
                (should (equal (last (car calls) 2) '("cross" "right")))
                (select-window top)
                (edger-up)
                (should (equal (last (car calls) 3) '("cross" "up" "split"))))
              (delete-other-windows)
              (let ((left (selected-window)) (right (split-window-right)))
                (select-window left)
                (edger-left)
                (should (equal (last (car calls) 3) '("cross" "left" "split")))
                (select-window right)
                (edger-right)
                (should (equal (last (car calls) 3) '("cross" "right" "split"))))))
        (set-frame-parameter frame 'edger-herdr-pane-id nil)))))
